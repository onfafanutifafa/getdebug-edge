...
| **Threads** | **Physical core count** (not logical CPUs, not `cores-1`) | Measured on an 8c/16t i9 (llama-bench, shipping Q4): 8 physical threads → **9.6 t/s** generation; 16 logical threads → **7.9 t/s** — physical cores are **~22% faster** for generation. Generation is memory-bandwidth-bound, so SMT siblings add contention and heat, not speed (prompt processing, which is compute-bound, is marginally faster at 16 threads — but generation is what the score measures). Fewer active execution units also run cooler; the profiler confirms **no throttling** (`throttled=false`, CPU p99 51.6%). |
