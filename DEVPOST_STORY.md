...
- **Model:** benchmarked **six** open models head-to-head (Qwen2.5-Coder-3B/1.5B, Qwen3-4B, Gemma-3-4B, Llama-3.2-3B, DeepSeek-Coder, GLM-Edge) on a real laptop. Qwen2.5-Coder-3B won on accuracy-per-resource.
- **Runtime:** llama.cpp with the model quantized to a ~2.1 GB 4-bit GGUF, loaded **once** into a persistent server (re-spawning per file was the biggest throughput/thermal hazard), context capped at 3072 tokens, KV cache quantized with flash attention.
...
- **Verification:** ran the whole thing in a container pinned to the contest spec — Ubuntu 22.04, 4 CPUs, **8 GB RAM with swap disabled** — reviewing a real Flask app.

## Challenges we ran into

- **Fine-tuning made it *worse* — twice.** Two LoRA runs on a free Colab T4 regressed recall (80%→50%→10% on our first corpus): the first learned terseness, the second overfit to templated data and started answering "no issues" on buggy code. The eval harness caught it, and we made the hard call to **ship the base model, not the fine-tune.**
...
- **Our own accuracy number was flattering us.** On 10 samples we saw 80% recall; expanding to 22 dropped it to a *truthful* 68%. Rather than hide that, we published it — then closed the gap to 82% with the hybrid detectors.
- **Small-model false positives are real** — on well-written code it over-flags (it called a correctly parameterized query "SQL injection"). We report our false-positive rate openly.
...
- **Intellectual honesty as a feature:** we expanded our own benchmark until the number got worse, reported it, and *then* engineered it back up — recall **68% → 82% with zero added false positives.**
- **We refused to ship a regression** even after investing in the fine-tune path, because a security tool that invents or misses bugs is worse than one that's honest about its limits.
...
---
<!-- ===== "Built with" tags (paste into the Built with field, up to 25) ===== -->
...
<!-- ===== "Try it out" links ===== -->
**Try it out:** https://github.com/onfafanutifafa/getdebug-edge  (flip the repo to public at submission time)
